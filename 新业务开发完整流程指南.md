...
### 2.3 创建 Servlet 控制器
**文件路径**: `src/org/cloudserviceengineering/app/business/[业务名称]Servlet.java`

**路径说明**:
- 所有业务相关的 Servlet 都放在 `src/org/cloudserviceengineering/app/business/` 目录
- 登录相关的 Servlet 放在 `src/org/cloudserviceengineering/app/login/` 目录
- 通过 `@WebServlet` 注解中的路径来区分不同角色，例如：
  - 管理员：`@WebServlet("/servlet/admin/...")`
  - 医生：`@WebServlet("/servlet/hospital/doctor/...")`
  - 护士：`@WebServlet("/servlet/hospital/nurse/...")`
  - 高管：`@WebServlet("/servlet/hospital/executive/...")`
  - 财务：`@WebServlet("/servlet/hospital/finance/...")`
  - 患者：`@WebServlet("/servlet/patient/...")`
...
### 3.1 创建 JSP 文件
**文件路径**: `WebRoot/business/html/[角色路径]/[业务组名称]/[页面名称].jsp`

**路径规范**:
- 根据角色确定路径：
  - 管理员：`WebRoot/business/html/admin/`
  - 医生：`WebRoot/business/html/doctor/`
  - 护士：`WebRoot/business/html/nurse/`
  - 高管：`WebRoot/business/html/executive/`
  - 财务：`WebRoot/business/html/finance/`
  - 患者：`WebRoot/business/html/patient/`
  - 企业：`WebRoot/business/html/enterprise/`
  - 高校：`WebRoot/business/html/university/`

**注意**: 所有业务页面使用 JSP 格式，只有登录页面使用 HTML 格式
...
<!-- 引入CSS和JS -->
<link rel="stylesheet" href="<%=basePath%>business/css/[业务名称].css">
<script src="<%=basePath%>business/scripts/business-common.js"></script>
<script src="<%=basePath%>business/scripts/[业务名称].js"></script>
```

**注意事项**:
- 使用 `basePath` 变量引入CSS和JS（确保路径正确）
- 列表容器必须有唯一ID（如：`id="data-list-body"`）
- 按钮和输入框建议添加 `aria-label` 和 `title` 属性（无障碍访问）
- JSP 文件需要包含 page 指令和 basePath 设置

**示例文件**:
- `WebRoot/business/html/executive/CooperationForum/forum-home.jsp`
- `WebRoot/business/html/nurse/nursing-service/daily-nursing.jsp`

### 3.2 创建 CSS 样式文件
**文件路径**: `WebRoot/business/css/[业务名称].css`

**路径说明**:
- 业务相关的 CSS 放在 `WebRoot/business/css/` 目录
- 非业务相关的 CSS（登录、注册、门户、首页）放在 `WebRoot/css/` 目录
...
**路径说明**:
- 业务相关的 JavaScript 放在 `WebRoot/business/scripts/` 目录
- 非业务相关的 JavaScript（登录、注册、首页）放在 `WebRoot/js/` 目录
...
**示例文件**:
- `WebRoot/business/scripts/forum.js` - 论坛业务脚本
- `WebRoot/business/scripts/nursing-service.js` - 护理服务业务脚本
- `WebRoot/js/login.js` - 登录表单验证（非业务相关）
- `WebRoot/js/register.js` - 注册表单验证（非业务相关）
...
const contentMapping = {
    // ... 已有映射
    '[内容ID]': '[JSP文件名].jsp', // 新增，注意使用 .jsp 扩展名
};
```
...
### 4.1 更新主页JSP
**文件路径**: `WebRoot/home/[角色路径]/[角色]-home.jsp`

**注意**: 主页文件是 JSP 格式，不是 HTML
...
### 5.2 功能测试
1. **访问主页**
   - 打开浏览器访问：`http://127.0.0.1:8099/E-HSS/home/[角色路径]/[角色]-home.jsp`
   - 检查导航是否正常显示
   - 检查 header.jsp 和 footer.jsp 是否正确加载
...
### 6.1 404 错误
- **问题**: 找不到 JSP/CSS/JS 文件
- **排查**:
  - 检查文件路径是否正确
  - 检查 `business-common.js` 中的路径映射（注意使用 .jsp 扩展名）
  - 检查 JSP 文件中的 `basePath` 是否正确设置
  - 检查 CSS/JS 引用路径是否使用了 `basePath` 变量
...
### 前端文件
1. **JSP 页面**: `WebRoot/business/html/[角色路径]/[业务组名称]/[页面名称].jsp`
2. **CSS 样式**: `WebRoot/business/css/[业务名称].css`（业务相关）
3. **JavaScript 脚本**: `WebRoot/business/scripts/[业务名称].js`（业务相关）

### 配置文件（修改）
1. **路由配置**: `WebRoot/business/scripts/business-common.js`（注意 contentMapping 使用 .jsp 扩展名）
2. **主页导航**: `WebRoot/home/[角色路径]/[角色]-home.jsp`
...
- [ ] JSP 文件已创建（结构完整，包含必要元素，使用 basePath）
...
**参考示例**:
- 论坛管理模块：`business/ForumServlet.java`、`ForumPostDAO.java`、`forum-home.jsp`、`forum.css`、`forum.js`
- 护理服务模块：`business/MedicalServiceServlet_Nurse.java`、`NursingTaskDAO.java`、`daily-nursing.jsp`、`nursing-service.css`、`nursing-service.js`

**重要提示**:
- 所有业务页面使用 JSP 格式（`.jsp`），只有登录页面使用 HTML 格式（`.html`）
- Servlet 分为两类：登录相关在 `app/login/`，业务相关在 `app/business/`
- CSS/JS 分为两类：业务相关在 `business/css/` 和 `business/scripts/`，非业务相关在 `WebRoot/css/` 和 `WebRoot/js/`
- 主页使用 JSP 格式，并通过 `<jsp:include>` 引入 `business/basic/header.jsp` 和 `footer.jsp`
